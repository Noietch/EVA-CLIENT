...
class ConsoleRequestHandler(BaseHTTPRequestHandler):
    ...
    def _stream_camera(self, key: str) -> None:
        ...
        try:
            while not ctx.runtime.transport.is_shutdown():
                ...
                calibrating = (
                    session is not None
                    and getattr(session, "active", False)
                    and key in session.camera_keys
                )
                if calibrating:
                    get_one = getattr(reader, "get_camera_frame", None)
                    if get_one is not None:
                    ...
                    if image is None:
                        ...
                        sig = (
                            "array",
                            arr.shape,
                            int(arr[::32, ::32].sum(dtype=np.int64)),
                            overlay_sig,
                        )
                        payload = _encode_jpeg(image, convert)
                else:
    ...
    def _post_calibrate_start(self, _body: dict) -> None:
        ...
        if cfg_poses:
            ...
            self.ctx.calibrate.poses = [
                list(map(float, np.asarray(p).reshape(-1))) for p in robot_poses
            ]
    ...
    def _post_calibrate_intrinsic_source(self, body: dict) -> None:
        ...
        if source not in INTRINSIC_SOURCES:
            self._send_json(
                400,
                {"ok": False, "error": f"source must be one of {INTRINSIC_SOURCES}"},
            )
            return
