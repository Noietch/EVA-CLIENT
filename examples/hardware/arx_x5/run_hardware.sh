#!/usr/bin/env bash
# Start the ARX X5 hardware node with its dedicated Python environment and vendored SDK.
# The script validates the runtime, prepares CAN and gripper state, then forwards CLI arguments
# to node.py; hardware state and local ZMQ ports are the only runtime side effects.
set -euo pipefail

cd "$(dirname "$0")/../../.."
ARX_X5_VENV_DIR="$PWD/examples/hardware/arx_x5/.venv"
...
ARX_X5_PORT_WAIT_SECONDS="5"
INIT_ARX_CAN="1"
ENABLE_ARX_ARMS="0"
RESET_ARX_CAN="${RESET_ARX_CAN:-0}"
RESET_ARX_X5_HARDWARE="${RESET_ARX_X5_HARDWARE:-0}"
CLEAR_ARX_X5_GRIPPER_ERRORS="${CLEAR_ARX_X5_GRIPPER_ERRORS:-1}"
ARX_X5_RESET_ATTEMPTS="${ARX_X5_RESET_ATTEMPTS:-3}"
ARX_X5_RESET_SETTLE_SECONDS="${ARX_X5_RESET_SETTLE_SECONDS:-1}"
DISABLED_ARMS_VALUE=""
# This machine uses the X5-2025 dual-rail gripper. SDK type 2 expects the
# physical gripper range [-3.4, 0.1] (open -> closed).
ARM_TYPE_VALUE="2"
ARX_X5_FILTER_SDK_BANNER="${ARX_X5_FILTER_SDK_BANNER:-1}"
SHOW_NODE_HELP=0

ARX_X5_NODE_PATH="examples/hardware/arx_x5/node.py"
...
warn() {
  printf '[arx_x5-run] WARN: %s\n' "$*" >&2
}

die() {
  printf '[arx_x5-run] ERROR: %s\n' "$*" >&2
  exit 1
}

print_node_help_fallback() {
  cat <<'EOF'
usage: node.py [-h] [--obs-endpoint OBS_ENDPOINT]
               [--action-endpoint ACTION_ENDPOINT]
               [--left-can-port LEFT_CAN_PORT]
               [--right-can-port RIGHT_CAN_PORT]
               [--alicia-port ALICIA_PORT]
               [--left-alicia-port LEFT_ALICIA_PORT]
               [--right-alicia-port RIGHT_ALICIA_PORT]
               [--alicia-reader {duo,sdk}] [--debug-alicia]
               [--collection-teleop-control] [--passive-collection]
               [--arm-type ARM_TYPE]
               [--gripper-open-pos GRIPPER_OPEN_POS]
               [--gripper-close-pos GRIPPER_CLOSE_POS]
               [--initial-gripper-scalar INITIAL_GRIPPER_SCALAR]
               [--start-at-zero]
               [--disabled-arm left_arm|right_arm] [--rate RATE]
               [--eva-config EVA_CONFIG]
               [--disabled-camera KEY]
               [--realsense-camera KEY=SERIAL_OR_INDEX]
...
run_python() {
  if [[ "${ARX_X5_FILTER_SDK_BANNER}" == "0" ]]; then
    "${ARX_X5_PYTHON_BIN}" -X faulthandler "$@"
    return
  fi
...
wait_for_startup_ports() {
  ...
  local deadline=$((SECONDS + ARX_X5_PORT_WAIT_SECONDS))
  local port occupied
  ...
  while true; do
    ...
    if [[ "${SECONDS}" -ge "${deadline}" ]]; then
      die "startup ports still in use after ${ARX_X5_PORT_WAIT_SECONDS}s: ${occupied[*]}"
    fi
    sleep 0.2
  done
}
...
stop_existing_hardware_node() {
  [[ "${RESET_ARX_X5_HARDWARE}" != "0" ]] || return 0
  ...
  for proc in /proc/[0-9]*; do
    ...
    case "${cmdline}" in
      *"${ARX_X5_NODE_PATH}"*) pids+=("${pid}") ;;
    esac
  done

  [[ "${#pids[@]}" -gt 0 ]] || {
    log "no existing ARX X5 hardware node to stop"
    return 0
  }

  log "stopping existing ARX X5 hardware node(s): ${pids[*]}"
  ...
  warn "ARX X5 hardware node did not stop after SIGINT; sending SIGTERM"
  ...
  die "existing ARX X5 hardware node did not stop: ${pids[*]}"
}

check_arx_x5_sdk() {
  local sdk_module python_version

  [[ -d "${ARX_X5_SDK_DIR}" ]] || die \
    "vendored official X5 SDK root not found: ${ARX_X5_SDK_DIR}; restore the tracked SDK files"
  sdk_module="$(find "${ARX_X5_BIMANUAL_DIR}" -maxdepth 1 -type f \
    -name '__init__.cpython-312-*.so' -print -quit 2>/dev/null || true)"
  [[ -n "${sdk_module}" ]] || die \
    "official X5 SDK-V2 CPython 3.12 module is missing under ${ARX_X5_BIMANUAL_DIR}"
  python_version="$("${ARX_X5_PYTHON_BIN}" -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')"
  [[ "${python_version}" == "3.12" ]] || die \
    "official X5 SDK-V2 requires Python 3.12; got ${python_version}; run examples/hardware/arx_x5/setup_env.sh"
  run_python -c "import bimanual; assert bimanual.__version__ == '1.0.0'"

  [[ -f "examples/hardware/arx_x5/robot.py" ]] || die \
    "ARX X5 hardware adapter is missing: examples/hardware/arx_x5/robot.py"
  [[ -f "${ARX_X5_NODE_PATH}" ]] || die "ARX X5 execution node is missing: ${ARX_X5_NODE_PATH}"
  log "ARX X5 SDK: official ARX X5 SDK-V2 1.0.0 root=${ARX_X5_SDK_DIR}"
}
...
reset_arx_x5_arms() {
  ...
  [[ "${ARX_X5_RESET_ATTEMPTS}" =~ ^[1-9][0-9]*$ ]] || \
    die "ARX_X5_RESET_ATTEMPTS must be a positive integer"
  ...
  ARX_RESET_ATTEMPTS="${ARX_X5_RESET_ATTEMPTS}" \
  ARX_RESET_SETTLE_SECONDS="${ARX_X5_RESET_SETTLE_SECONDS}" \
  run_python - <<'PY'
import os
import time
...
from examples.hardware.arx_x5.robot import ARX_X5_ARM_JOINT_LIMITS
...
for spec in os.environ["ARX_RESET_SPECS"].split(";"):
    ...
    if group_name in disabled:
        print(f"[arx_x5-reset] {group_name} skipped: disabled", flush=True)
        continue
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            print(
                f"[arx_x5-reset] {group_name} attempt {attempt}/{attempts}: "
            ...
            print(
                f"[arx_x5-reset] {group_name}: offline_joints={joint_names(offline)} "
            ...
            if healthy and group_name == "right_arm":
                ...
                probe_step = 0.01
                if probe_target[1] + probe_step > ARX_X5_ARM_JOINT_LIMITS[1, 1] - 0.005:
                ...
                print(
                    f"[arx_x5-reset] right_arm J2 probe: accepted={accepted!r} "
            ...
            print(f"[arx_x5-reset] {group_name}: {last_error}", flush=True)
        finally:
            if arm is not None:
                try:
                    arm.disable()
                    ...
                    print(f"[arx_x5-reset] {group_name}: disable failed: {exc}", flush=True)
                try:
                    arm.close()
                except Exception as exc:
                    cleanup_errors.append(f"close failed: {exc}")
                    print(f"[arx_x5-reset] {group_name}: close failed: {exc}", flush=True)
        if cleanup_errors:
    ...
    if last_error:
        ...
        print(f"[arx_x5-reset] {group_name} reset passed", flush=True)

if failed:
    raise SystemExit("X5 reset failed: " + "; ".join(failed))

print("[arx_x5-reset] both enabled arms passed; starting hardware node", flush=True)
PY
}

clear_arx_x5_gripper_errors() {
  [[ "${CLEAR_ARX_X5_GRIPPER_ERRORS}" != "0" ]] || return 0
...
from examples.hardware.arx_x5.sdk import clear_gripper_error
...
for spec in os.environ["ARX_GRIPPER_SPECS"].split(";"):
    group_name, can_port = spec.split(":", 1)
    if group_name in disabled:
        print(f"[arx_x5-run] {group_name} gripper fault clear skipped: disabled", flush=True)
        continue
    result = clear_gripper_error(can_port)
    print(
        f"[arx_x5-run] {group_name} gripper state cleared on {can_port}: "
...
init_arx_can() {
  ...
  if [[ "${#mappings[@]}" -eq 0 ]]; then
    log "CAN setup skipped: all ARX X5 arms are disabled"
    return
  fi
...
for spec in os.environ["ARX_ENABLE_SPECS"].split(";"):
    group_name, can_port = spec.split(":", 1)
    if group_name in disabled:
        print(f"[arx_x5-run] {group_name} enable skipped: disabled")
        continue
    ...
    enabled += 1
    print(f"[arx_x5-run] {group_name} enabled on {can_port} type={arm_type}")
    arm.protect_mode()
    arm.close()

if enabled == 0:
    print("[arx_x5-run] no ARX X5 arms enabled")
PY
}
...
for arg in "$@"; do
  case "${arg}" in
    -h|--help)
      ...
      ENABLE_ARX_ARMS=0
      RESET_ARX_CAN=0
      RESET_ARX_X5_HARDWARE=0
      CLEAR_ARX_X5_GRIPPER_ERRORS=0
      SHOW_NODE_HELP=1
      ;;
  esac
done
...
if [[ "${SHOW_NODE_HELP}" == "0" ]]; then
  [[ -x "${ARX_X5_PYTHON_BIN}" ]] || {
    printf '[arx_x5-run] ERROR: ARX X5 environment not found: %s; run examples/hardware/arx_x5/setup_env.sh\n' \
      "${ARX_X5_VENV_DIR}" >&2
    exit 1
  }
fi
...
run_python "${ARX_X5_NODE_PATH}" \
  ...
  --eva-config "configs/02_collection/arx_x5.py" \
  --rate "100" \
