...
from examples.hardware.x5.camera import (  # noqa: E402
    X5_LIGHTING_PROFILES,
...
from examples.hardware.x5.robot import (  # noqa: E402
    ARM_DOF,
    DEFAULT_ARM_TYPE,
...
class X5ZmqConfig:
    ...
    realsense_cameras: tuple[RealSenseCameraSpec, ...]
...
class ArxTeleopSource:
    def __init__(self, config: X5ZmqConfig) -> None:
        self._config = config
...
class X5ZmqNode:
    """Bridge EVA ZMQ wire messages to an X5-controlled dual-arm pair."""

    def __init__(self, config: X5ZmqConfig) -> None:
        self._config = config
        # JAX initializes and compiles lazily. Warm up the collection FK path before
        # the X5 SDK and RealSense start their native worker threads.
        logger.info("Initializing X5 forward kinematics")
        self._fk_solver = self._build_fk_solver()
        logger.info("X5 forward kinematics ready")
        ...
        logger.info(
            "X5 ZMQ node ready: obs_pub=%s action_sub=%s can_ports=%s "
    ...
    def _build_camera_cache(self, config: X5ZmqConfig) -> Any:
        ...
        return RealSenseCameraCache(config.realsense_cameras)
    ...
    def start_collection(self, control_source: str = COLLECTION_CONTROL_TRANSPORT) -> None:
        if control_source not in COLLECTION_CONTROL_SOURCES:
            raise ValueError(f"Unsupported X5 collection control source: {control_source!r}")
        ...
        if control_source == COLLECTION_CONTROL_CLIENT:
            self._collection_active = True
            logger.info("X5 collection started with EVA client control")
            return
        try:
    ...
    def start_hil(self, mode: str) -> None:
        if self._config.passive_collection:
            self._hil_error = "X5 passive collection mode cannot control HIL"
            return
    ...
    def _fk(self, qpos: np.ndarray) -> np.ndarray:
        if self._fk_solver is None:
            ...
            self._fk_solver = self._build_fk_solver(
                [live_state[group_name] for group_name in GROUP_NAMES]
            )
    ...
    def _build_fk_solver(
        ...
        if initial_qpos_groups is None:
            ...
            if len(groups) != len(GROUP_NAMES) or any(
                ...
                raise ValueError("X5 FK seed must contain one finite 7D state for each arm group")
    ...
    def _reseed_fk_solver_from_live_state(self) -> None:
        state = self._robot.read_state()
        solver = self._build_fk_solver([state[group_name] for group_name in GROUP_NAMES])
        previous_solver = self._fk_solver
        self._fk_solver = solver
        if previous_solver is not None:
            previous_solver.close()
        logger.info(
            "X5 forward kinematics reseeded from live state: left=%s right=%s",
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_false",
        help="Drive X5 arms from Alicia-D during collection; this is the default.",
    )
    parser.add_argument(
        "--passive-collection",
        dest="passive_collection",
        action="store_true",
        help="Only observe X5 arms/cameras during collection; do not drive X5 from Alicia-D.",
    )
    ...
    parser.add_argument(
        ...
        help="X5 SDK arm type code: 0 = X5 2023, 2 = X5 2025.",
    )
    parser.add_argument(
        "--gripper-open-pos",
        type=float,
        default=DEFAULT_GRIPPER_OPEN_POS,
        help="X5 SDK gripper position that maps to normalized scalar 1.0.",
    )
    parser.add_argument(
        "--gripper-close-pos",
        type=float,
        default=DEFAULT_GRIPPER_CLOSE_POS,
        help="X5 SDK gripper position that maps to normalized scalar 0.0.",
    )
    parser.add_argument(
        "--initial-gripper-scalar",
        type=float,
        default=None,
        help="Initial gripper scalar to command on first X5 connection; 1.0 is fully open.",
    )
    parser.add_argument(
        "--start-at-zero",
        action="store_true",
        help="Move both X5 arms to the all-zero joint position during startup.",
    )
    parser.add_argument(
        "--disabled-arm",
        action="append",
        ...
        help="Intentionally skip one X5 arm; repeatable.",
    )
...
def build_config(args: argparse.Namespace) -> X5ZmqConfig:
    can_ports = {
    ...
    resolution = parse_resolution(args.realsense_resolution)
    default_cameras = default_realsense_camera_specs(
        resolution=resolution,
        fps=int(args.realsense_fps),
        timeout_ms=int(args.realsense_timeout_ms),
        profile=args.realsense_profile,
    )
    override_cameras = parse_realsense_camera_specs(
    ...
    cameras = merge_realsense_camera_specs(
        default_cameras,
        override_cameras,
...
def main() -> None:
    ...
    node: X5ZmqNode | None = None
    try:
        config = build_config(args)
        node = X5ZmqNode(config)
        ...
        logger.exception("X5 hardware node failed")
        raise
    finally:
        if node is not None:
            node.close()
