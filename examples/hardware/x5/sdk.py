"""Thin adapter around the official ARX5_beta SDK-V2 package."""
...
X5_2025_GRIPPER_MOTOR_ID = 8
X5_MOTOR_CLEAR_ERROR_COMMAND = b"\xff\xff\xff\xff\xff\xff\xff\xfb"
...
def clear_gripper_error(can_port: str, *, timeout_s: float = 1.0) -> dict[str, Any]:
    ...
    try:
        bus.open()
        gripper = MotorB(X5_2025_GRIPPER_MOTOR_ID, bus)
        initial_rx_count = int(gripper.rx_count)
        if not bus.send(X5_2025_GRIPPER_MOTOR_ID, X5_MOTOR_CLEAR_ERROR_COMMAND):
