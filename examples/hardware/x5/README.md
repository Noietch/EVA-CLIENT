...
bash examples/hardware/x5/setup_env.sh
```

脚本会自动完成以下操作：

1. 创建 `examples/hardware/x5/.venv`。
2. 安装 `pyrealsense2` 和 X5 硬件节点依赖。
3. 使用仓库内置的官方
   [ARX5_beta SDK-V2](https://github.com/ARXroboticsX/ARX5_beta)，当前同步到
   `1aa8a7d2ddefced2229f953feffc248be1c0b44d`，路径为
   `examples/hardware/x5/SDK/X5`。
4. 校验官方 CPython 3.12 二进制和 FK/IK，并安装 `pin`、`python-can`、
   `ruckig` 等官方依赖。

EVA Client 中新增的 X5 集成代码遵循仓库根目录的 Apache-2.0 许可证。
`ARX5_beta` 是独立的第三方 submodule，不在 EVA Client 的 Apache-2.0
...
```bash
bash examples/hardware/x5/run_hardware.sh
```
...
```bash
bash examples/hardware/x5/run_fake.sh
```

需要模拟较平滑的机械响应时，可以切换回每个 qpos 独立的二阶系统：

```bash
bash examples/hardware/x5/run_fake.sh --dynamics-mode second-order
```
...
```bash
bash examples/hardware/x5/run_hardware.sh --disabled-arm left_arm
```

若启动时有单关节不上电，执行故障恢复启动：

```bash
bash examples/hardware/x5/reset_hardware.sh
```
...
`None`，且右 J2 微动通过，才会继续启动和回零。可用 `X5_RESET_ATTEMPTS`
调整次数：

```bash
X5_RESET_ATTEMPTS=5 bash examples/hardware/x5/reset_hardware.sh
```

如果需要让双臂在节点启动时移动到六轴全 0 的位置，可使用：

```bash
bash examples/hardware/x5/run_hardware.sh --start-at-zero
```
...
```bash
bash examples/hardware/x5/run_hardware.sh --help
```
