"""ARX X5 dual-arm adapter backed by the native X5 motor interface.
...
DEFAULT_X5_TYPE = 2
DEFAULT_ARM_TYPE = DEFAULT_X5_TYPE

# The normalized EVA/R5 convention is 0.0 = closed and 1.0 = open. Keep the
# physical X5 endpoints explicit because its SDK position increases toward closed.
X5_2023_GRIPPER_OPEN_POS = 0.0
X5_2023_GRIPPER_CLOSE_POS = 5.0
X5_2025_GRIPPER_OPEN_POS = -3.4
X5_2025_GRIPPER_CLOSE_POS = 0.1
DEFAULT_GRIPPER_OPEN_POS = X5_2025_GRIPPER_OPEN_POS
DEFAULT_GRIPPER_CLOSE_POS = X5_2025_GRIPPER_CLOSE_POS
X5_GRIPPER_COMMAND_DELTA = 0.02
X5_RETRY_INTERVAL_SEC = 5.0
X5_HOME_DURATION_S = 5.0
X5_INITIAL_ARM_QPOS = np.asarray((0.0, 1.58, 0.49, 1.15, 0.0, 0.0), dtype=np.float64)
X5_SAFETY_WARNING_INTERVAL_SEC = 2.0

# ARX5_beta SDK-V2 physical limits, converted from the documented degree ranges.
X5_ARM_JOINT_LIMITS = np.asarray(
    [
...
def initial_qpos(
    ...
    arm_target = X5_INITIAL_ARM_QPOS if arm_qpos is None else np.asarray(arm_qpos, dtype=np.float64)
    if arm_target.shape != (ARM_DOF,) or not np.all(np.isfinite(arm_target)):
        raise ValueError(f"Expected finite X5 arm qpos shaped ({ARM_DOF},), got {arm_target}")
...
def clip_group_action(action: np.ndarray) -> np.ndarray:
    """Clip one X5 7D command to the normalized gripper range."""
    vector = np.asarray(action, dtype=np.float32)
    if vector.shape[0] < GROUP_DOF:
        raise ValueError(f"Expected X5 action with at least {GROUP_DOF} values, got {vector.shape}")
    clipped = vector[:GROUP_DOF].copy()
    if not np.all(np.isfinite(clipped)):
        raise ValueError("X5 group action contains NaN or infinity")
...
def split_dual_action(action: np.ndarray) -> dict[str, np.ndarray]:
    """Split EVA's 14D command into left and right 7D X5 commands."""
    vector = np.asarray(action, dtype=np.float32)
...
def limit_arm_command(target_qpos: np.ndarray) -> tuple[np.ndarray, bool]:
    """Validate and clip one six-joint command to the official X5 software limits."""
    target = np.asarray(target_qpos, dtype=np.float64)
    if target.shape != (ARM_DOF,):
        raise ValueError(f"Expected X5 arm qpos shaped ({ARM_DOF},), got {target.shape}")
    if not np.all(np.isfinite(target)):
        raise ValueError("X5 arm qpos contains NaN or infinity")
    limited = np.clip(
        target,
        X5_ARM_JOINT_LIMITS[:, 0],
        X5_ARM_JOINT_LIMITS[:, 1],
    )
...
def normalize_group_name(group_name: str) -> str:
    ...
    if normalized not in GROUP_NAMES:
        raise ValueError(f"Unknown X5 arm {group_name!r}; expected one of {GROUP_NAMES}")
    return normalized
...
class X5DualArm:
    """Dual X5 state reader and command publisher backed by two SingleArm objects."""

    def __init__(self, config: X5ArmConfig) -> None:
        self._config = config
        ...
        if not np.isfinite(publish_rate_hz) or publish_rate_hz <= 0:
            raise ValueError(f"Invalid X5 publish rate: {publish_rate_hz}")
        self._action_duration_s = 1.0 / publish_rate_hz

        configured_type = getattr(config, "arm_type", None)
        if configured_type is None:
            configured_type = getattr(config, "x5_type", DEFAULT_X5_TYPE)
        self._x5_type = int(configured_type)
        if self._x5_type == 2:
            default_open_pos = X5_2025_GRIPPER_OPEN_POS
            default_close_pos = X5_2025_GRIPPER_CLOSE_POS
        else:
            default_open_pos = X5_2023_GRIPPER_OPEN_POS
            default_close_pos = X5_2023_GRIPPER_CLOSE_POS
        ...
        if not self._start_at_zero:
            self._startup_arm_qpos[:] = X5_INITIAL_ARM_QPOS
        ...
        if self._disabled_groups:
            logger.info("Disabled X5 arms: %s", sorted(self._disabled_groups))
    ...
    def _connect(self, group_name: str) -> Any | None:
        ...
        try:
            ...
            if self._initial_gripper_scalar is not None:
                ...
                try:
                    ...
                    logger.debug(
                        "Failed to read initial X5 %s gripper position: %s", group_name, exc
                    )
                else:
        ...
        logger.info(
            "Connected and enabled %s through official X5 SDK-V2: can_port=%s type=%d",
            group_name,
            can_port,
            self._x5_type,
        )
    ...
    def _mark_offline(self, group_name: str, exc: Exception) -> None:
        self._offline_until[group_name] = time.monotonic() + X5_RETRY_INTERVAL_SEC
        ...
        if arm is not None:
            try:
                ...
                pass
        logger.warning(
            "X5 %s offline; using cached qpos for %.1f s: %s",
            group_name,
            X5_RETRY_INTERVAL_SEC,
            exc,
        )
    ...
    def _read_group_currents(self, group_name: str, arm: Any) -> None:
        try:
            ...
            logger.debug("Failed to read X5 %s joint currents: %s", group_name, exc)
            return
    ...
    def slow_home(self, should_stop: Any | None = None) -> bool:
        ...
        for group_name in GROUP_NAMES:
            ...
            arm = self._connect(group_name)
            if arm is None:
                raise RuntimeError(f"Cannot home offline X5 arm: {group_name}")
            ...
            if fault is not None or offline_joints:
                ...
                raise RuntimeError(
                    f"Refusing to home unhealthy X5 arm {group_name}: {', '.join(details)}"
                )
            ...
            if positions.shape != (ARM_DOF,) or not np.all(np.isfinite(positions)):
                raise RuntimeError(f"Invalid live X5 qpos for {group_name}: {positions}")
            starts[group_name] = positions
            arms[group_name] = arm

        duration_s = X5_HOME_DURATION_S
        target_name = "zero" if self._start_at_zero else "home"
        logger.info(
            "Official X5 %s trajectory starting: duration=%.2f s",
            target_name,
            duration_s,
        )
        ...
        while time.monotonic() < deadline:
            if should_stop is not None and should_stop():
                logger.info("Official X5 %s trajectory interrupted", target_name)
                return False
        ...
        logger.info("Official X5 %s trajectory complete", target_name)
        return True
    ...
    def _warn_safety(self, group_name: str, message: str) -> None:
        ...
        if (
            ...
            < X5_SAFETY_WARNING_INTERVAL_SEC
        ):
            return
        self._last_safety_warning_time[group_name] = now
        logger.warning("X5 %s safety limiter: %s", group_name, message)
    ...
    def _publish_gripper_if_needed(self, group_name: str, gripper_scalar: float) -> bool:
        if group_name in self._last_gripper_command:
            ...
            if delta < X5_GRIPPER_COMMAND_DELTA:
                return True
    ...
    def go_home(self) -> dict[str, bool]:
        """Send each connected X5 arm back to its SDK home pose."""
        ...
        for group_name in GROUP_NAMES:
            ...
            if arm is None:
                ...
                continue
            try:
                arm.set_joint_positions(X5_INITIAL_ARM_QPOS.tolist(), duration=2.0)
            except Exception as exc:
                self._mark_offline(group_name, exc)
                results[group_name] = False
                continue
            offset = self._offset(group_name)
            with self._lock:
                self._qpos[offset : offset + ARM_DOF] = X5_INITIAL_ARM_QPOS
            results[group_name] = True
            logger.info("Sent %s to X5 home pose", group_name)
        return results
    ...
    def close(self) -> None:
        ...
        for group_name, arm in list(self._arms.items()):
            try:
                ...
                logger.debug("Failed to stop X5 %s cleanly: %s", group_name, exc)
            finally:
                try:
                    arm.close()
                except Exception as exc:
                    logger.debug("Failed to close X5 %s SDK worker: %s", group_name, exc)
        self._arms.clear()
...
__all__ = [
    ...
    "GRIPPER_INDEX",
    "TOTAL_DOF",
    "X5_2023_GRIPPER_CLOSE_POS",
    "X5_2023_GRIPPER_OPEN_POS",
    "X5_2025_GRIPPER_CLOSE_POS",
    "X5_2025_GRIPPER_OPEN_POS",
    "X5_ARM_JOINT_LIMITS",
    "X5_HOME_DURATION_S",
    "X5_INITIAL_ARM_QPOS",
    "X5ArmConfig",
