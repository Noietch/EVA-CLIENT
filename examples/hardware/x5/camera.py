"""Intel RealSense D405 color-camera cache for the X5 hardware node."""
...
X5_CAMERA_KEYS: tuple[str, ...] = ("cam_high", "cam_left_wrist", "cam_right_wrist")
X5_LIGHTING_PROFILES: tuple[str, ...] = ("day", "night")
DEFAULT_X5_D405_CAMERAS: dict[str, str] = {
    "cam_high": "409122271504",
    "cam_left_wrist": "352122272510",
    "cam_right_wrist": "352122271326",
}
DEFAULT_X5_D405_PROFILE_PATH = Path(__file__).with_name("d405_profile.yaml")
...
def load_realsense_color_profiles(
    path: Path = DEFAULT_X5_D405_PROFILE_PATH,
    profile: str = "night",
    ...
    if profile not in X5_LIGHTING_PROFILES:
        raise ValueError(
            f"Unknown RealSense lighting profile {profile!r}; "
            f"expected one of {X5_LIGHTING_PROFILES}"
        )
    ...
    legacy_format = set(raw) == set(X5_CAMERA_KEYS)
    if legacy_format:
        if profile != "night":
            raise ValueError(
        ...
        unknown = set(raw) - set(X5_LIGHTING_PROFILES)
        missing = set(X5_LIGHTING_PROFILES) - set(raw)
    ...
    unknown = set(selected) - set(X5_CAMERA_KEYS)
    missing = set(X5_CAMERA_KEYS) - set(selected)
    if unknown or missing:
        raise ValueError(
    ...
    for image_key in X5_CAMERA_KEYS:
...
def parse_realsense_camera_specs(
    ...
    *,
    ...
    for raw in specs:
        ...
        if image_key not in X5_CAMERA_KEYS:
            raise ValueError(
                f"Unknown X5 camera key {image_key!r}; expected one of {X5_CAMERA_KEYS}"
            )
        if not selector:
...
def default_realsense_camera_specs(
    *,
    ...
    return tuple(
        ...
        for image_key, serial in DEFAULT_X5_D405_CAMERAS.items()
    )
