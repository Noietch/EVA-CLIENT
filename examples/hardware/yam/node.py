#!/usr/bin/env python3
"""YAM YAM execution-layer node for EVA's ZMQ transport."""
...
def split_group_eef(
    ...
    if vector.shape != (expected,):
        raise ValueError(f"Expected a {expected}-D YAM EEF vector, got {vector.shape}")
    return {
...
class YamZmqConfig:
    ...
    gripper_close_torque_limit: float
    gripper_open_torque_limit: float
...
def _parse_group_map(
    ...
    for value in values:
        ...
        if group_name not in group_names:
            raise ValueError(f"Unknown YAM group {group_name!r}; expected one of {group_names}")
        if not interface:
...
class YamZmqNode:
    ...
    def __init__(self, config: YamZmqConfig, camera_endpoint: str = "") -> None:
        ...
        self._followers = YamFollowers(config)
        ...
        logger.info(
            "YAM node ready: robot=%s followers=%s leaders=%s",
            config.robot_name,
    ...
    def _ensure_direct_leader_control(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "YAM direct leader control is not ready; retrying in %.1f s: %s",
    ...
    def _stop_collection(self) -> None:
        ...
        logger.info("YAM leader-follower collection stopped")
    ...
    def _start_hil(self, mode: str) -> None:
        ...
        try:
            ...
            self._hil_error = str(exc)
            self._hil_active = False
            logger.exception("Could not start YAM HIL")
            return
        self._hil_mode = mode
        self._hil_error = ""
        self._hil_active = True
        logger.info("YAM HIL started: mode=%s", mode)
    ...
    def _drain_actions(self) -> None:
        while True:
            ...
            try:
                ...
                logger.warning("Dropped malformed YAM action: %s", exc)
                continue
    ...
    def _leader_action(self) -> np.ndarray | None:
        ...
        try:
            ...
            self._hil_error = str(exc)
            logger.warning("YAM leader read/control failed: %s", exc)
    ...
    def _update_operator_buttons(
        ...
        logger.info(
            "YAM leader %s button: event=%s id=%d",
            button_name,
            event,
    ...
    def _publish_observation(self) -> None:
        ...
        if publisher is None:
            raise RuntimeError("YAM observation publisher is not ready")
    ...
    def _log_status_if_due(self) -> None:
        ...
        logger.info(
            "YAM status: arms=%s cameras=%s obs=%.1fHz actions=%.1fHz "
    ...
    def _publish_loop(self) -> None:
        ...
        try:
            ...
            self._stop.set()
            logger.exception("YAM observation publisher failed")
        finally:
            self._publisher_ready.set()
            self._obs_pub = None
    ...
    def serve_forever(self) -> None:
        ...
        self._publisher_thread = threading.Thread(
            ...
            name="yam-observation-publisher",
            daemon=True,
        )
        self._publisher_thread.start()
        if not self._publisher_ready.wait(timeout=2.0):
            raise RuntimeError("YAM observation publisher did not start")
        if self._publisher_error is not None:
            raise RuntimeError(
                "YAM observation publisher failed to start"
        ...
        if self._publisher_error is not None:
            raise RuntimeError(
                "YAM observation publisher stopped unexpectedly"
            ) from self._publisher_error
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gripper-close-torque-limit",
        type=float,
        default=0.29,
        help="Maximum complete gripper closing torque command in Nm; in (0, 1].",
    )
    parser.add_argument(
        "--gripper-open-torque-limit",
        type=float,
        default=0.29,
        help="Maximum complete gripper opening torque command in Nm; in (0, 1].",
    )
    parser.add_argument(
        "--gripper-kp",
        type=float,
        default=5.0,
        help="Gripper software spring stiffness in Nm/rad; must be in (0, 20].",
    )
    parser.add_argument(
        "--gripper-damping",
        type=float,
        default=0.5,
        help="Gripper software damping in Nm/(rad/s); must be in (0, 5].",
    )
    parser.add_argument(
        "--gripper-max-speed",
        type=float,
...
def build_config(args: argparse.Namespace) -> YamZmqConfig:
    ...
    for name in ("gripper_close_torque_limit", "gripper_open_torque_limit"):
        value = getattr(args, name)
        if not np.isfinite(value) or not 0 < value <= 1:
            raise ValueError(f"--{name.replace('_', '-')} must be finite and in (0, 1]")
    if not np.isfinite(args.gripper_damping) or not 0 < args.gripper_damping <= 5:
        raise ValueError("--gripper-damping must be finite and in (0, 5]")
    if not np.isfinite(args.gripper_kp) or not 0 < args.gripper_kp <= 20:
        raise ValueError("--gripper-kp must be finite and in (0, 20]")
    ...
    return YamZmqConfig(
        ...
        gripper_damping=float(args.gripper_damping),
        gripper_close_torque_limit=float(args.gripper_close_torque_limit),
        gripper_open_torque_limit=float(args.gripper_open_torque_limit),
