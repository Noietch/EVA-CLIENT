"""YAM YAM follower and teaching-handle adapters for the EVA execution node."""
...
from examples.hardware.yam.wire import WireAction
...
def _ensure_motor_chain_running(robot: Any, label: str) -> None:
    ...
    if getattr(motor_chain, "running", True) is False:
        raise RuntimeError(f"YAM {label} CAN motor loop stopped")
...
def split_action(action: np.ndarray, group_names: tuple[str, ...]) -> dict[str, np.ndarray]:
    ...
    if vector.shape != (expected,):
        raise ValueError(f"Expected a {expected}-D YAM action, got {vector.shape}")
    return {
...
def _sdk_factory(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "YAM SDK is unavailable; run examples/hardware/yam/setup_sdk.sh"
        ) from exc
...
class YamFollowers:
    ...
    def __init__(self, config: YamArmConfig, factory: RobotFactory | None = None) -> None:
        self._config = config
    ...
    def _connect(self, group_name: str) -> Any | None:
        ...
        try:
            ...
            if self._config.end_effector_mass is not None:
                logger.info(
                    "Set YAM follower %s end-effector model mass to %.3f kg",
                    group_name,
                    self._config.end_effector_mass,
                )
            if self._config.gravity_comp_factor is not None:
                logger.info(
                    "Set YAM follower %s gravity compensation factors to %s",
                    group_name,
            ...
            if robot is not None:
                try:
                    ...
                    logger.debug(
                        "Failed to close rejected YAM %s",
                        group_name,
                        exc_info=True,
                    )
    ...
    def move_to_startup_position(self) -> None:
        ...
        if duration_s <= 0:
            raise ValueError("YAM startup duration must be positive")
        missing = [name for name in self._config.group_names if name not in self._robots]
        if missing:
            raise RuntimeError(f"Cannot move offline YAM followers to zero: {missing}")
        ...
        for step in range(1, steps + 1):
            ...
            for group_name, robot in tuple(self._robots.items()):
                ...
                try:
                    ...
                    self._mark_offline(group_name, exc)
                    raise RuntimeError(
                        f"YAM follower {group_name} failed while moving to zero"
                    ) from exc
            if step < steps:
        ...
        if self._config.tracking_ki > 0 and self._config.startup_trim_duration_s > 0:
            ...
            settled_steps = 0
            logger.info(
                "Settling YAM all-zero target with integral trim: "
            ...
            for step in range(trim_steps):
                ...
                for group_name, robot in tuple(self._robots.items()):
                    try:
                        ...
                        self._mark_offline(group_name, exc)
                        raise RuntimeError(
                            f"YAM follower {group_name} failed while trimming zero"
                        ) from exc
        ...
        log(
            "YAM all-zero move settled: residuals_rad=%s max_error=%.5f trim_rad=%s",
            residuals,
            max_error,
            self.tracking_trim(),
        )
    ...
    def watchdog_tick(self) -> None:
        ...
        logger.warning(
            "YAM command watchdog expired after %.3f s; followers entered %s idle",
    ...
    def close(self) -> None:
        for group_name, robot in tuple(self._robots.items()):
            try:
                ...
                logger.exception("Failed to close YAM follower %s", group_name)
        self._robots.clear()
...
class YamLeaders:
    ...
    def connect(self) -> None:
        ...
        if tuple(self._config.group_names) != configured or set(
            ...
            raise RuntimeError("YAM leader control requires both dual-YAM leader CAN interfaces")
        leader_interfaces = tuple(self._config.leader_can_channels[name] for name in configured)
        if len(set(leader_interfaces)) != 2:
            raise RuntimeError("YAM leader control requires two distinct CAN interfaces")
        reused_interfaces = set(leader_interfaces) & set(
            self._config.follower_can_channels.values()
        )
        if reused_interfaces:
            raise RuntimeError(
                f"YAM leader and follower CAN interfaces overlap: {sorted(reused_interfaces)}"
            )
        try:
            for name in configured:
                ...
                logger.info(
                    "Connected YAM leader %s on %s with gravity compensation active",
                    name,
    ...
    def move_to_startup_position(self) -> None:
        ...
        steps = max(int(round(duration_s * control_hz)), 1)
        logger.info(
            "Moving YAM leaders to all-zero arm position over %.1f s",
            duration_s,
        )
        try:
            for step in range(1, steps + 1):
            ...
            self.close()
            raise
        logger.info("YAM leaders reached zero and returned to gravity compensation")
    ...
    def _read_gripper(self, group_name: str, robot: Any) -> float:
        ...
        if not encoder_states:
            raise RuntimeError("YAM teaching-handle encoder state is not ready")
        ...
        if raw_position is None:
            raise RuntimeError(
                "YAM SDK does not expose raw teaching-handle encoder positions; "
    ...
    def read_action(self) -> np.ndarray:
        ...
        for name in self._config.group_names:
            ...
            if arm.shape != (ARM_DOF,):
                raise ValueError(f"YAM leader {name} returned shape {arm.shape}")
    ...
    def close(self) -> None:
        for name, robot in tuple(self._robots.items()):
            try:
                ...
                logger.exception("Failed to close YAM leader %s", name)
        self._robots.clear()
