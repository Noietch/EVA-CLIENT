...
ARX_R5_CAMERA_KEYS: tuple[str, ...] = ("cam_high", "cam_left_wrist", "cam_right_wrist")
DEFAULT_ARX_R5_ORBBEC_CAMERAS: dict[str, str | None] = {
...
def default_arx_r5_orbbec_camera_specs(
    resolution: tuple[int, int] | None = None,
    fps: int = 30,
    color_format: str = "RGB",
    timeout_ms: int = 1000,
    ...
    return tuple(
        ...
        for image_key, serial in DEFAULT_ARX_R5_ORBBEC_CAMERAS.items()
    )
...
def merge_orbbec_camera_specs(
    ...
    for spec in defaults + overrides:
        if spec.image_key not in ARX_R5_CAMERA_KEYS:
            raise ValueError(
                f"Unknown ARX camera key {spec.image_key!r}; expected one of {ARX_R5_CAMERA_KEYS}"
            )
...
def get_orbbec_sdk() -> ModuleType:
    try:
        ...
        raise RuntimeError(
            "pyorbbecsdk could not be imported. Activate the ARX R5 hardware "
            "environment before running examples/hardware/arx_r5/run_hardware.sh."
        ) from exc
...
def _cv2() -> Any:
    try:
        ...
        raise RuntimeError(
            "OpenCV could not be imported. Activate the ARX R5 hardware environment first."
        ) from exc
    return cv2
