#!/usr/bin/env bash
# Start the real ARX R5 ZMQ node from this adapter's isolated Python environment.
# Inputs come from the ARX/CAN/camera environment variables and forwarded CLI arguments.
# The script validates the runtime, prepares SDK paths and CAN devices, then runs node.py.
# Side effects include configuring CAN interfaces and commanding enabled robot arms.
set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "$0")" && pwd)"
REPO_DIR="$(cd -- "$SCRIPT_DIR/../../.." && pwd)"
ARX_R5_DIR="${REPO_DIR}/examples/hardware/arx_r5"
ARX_R5_VENV_DIR="${ARX_R5_DIR}/.venv"
ARX_R5_PYTHON_BIN="${ARX_R5_VENV_DIR}/bin/python"
[[ -x "${ARX_R5_PYTHON_BIN}" ]] || {
  printf '[arx_r5-run] ERROR: ARX R5 environment not found: %s; run examples/hardware/arx_r5/setup_env.sh\n' \
    "${ARX_R5_VENV_DIR}" >&2
  exit 1
}
...
# The ARX R5 SDK builds a pybind module plus shared libraries. Put its package
# root on PYTHONPATH and its library dirs on the loader path so imports resolve.
ARX_R5_SDK_DIR="${ARX_R5_SDK_DIR:-$ARX_R5_DIR/SDK/ARX_R5_python}"
ARX_R5_LIBRARY_PATH="$ARX_R5_SDK_DIR/bimanual/lib:$ARX_R5_SDK_DIR/bimanual/lib/arx_r5_src"
export PYTHONPATH="$ARX_R5_SDK_DIR:$PYTHONPATH"
...
ARX_R5_CAN_SLCAND_SPEED="${ARX_R5_CAN_SLCAND_SPEED:-s8}"
ARX_R5_CAN_WAIT_SECONDS="${ARX_R5_CAN_WAIT_SECONDS:-5}"
INIT_ARX_R5_CAN="${INIT_ARX_R5_CAN:-1}"
ENABLE_ARX_R5_ARMS="${ENABLE_ARX_R5_ARMS:-0}"
DISABLED_ARMS_VALUE="${DISABLED_ARMS:-}"
ARX_R5_FILTER_SDK_BANNER="${ARX_R5_FILTER_SDK_BANNER:-1}"
ORBBEC_WHITE_BALANCE_FILE="${ORBBEC_WHITE_BALANCE_FILE-examples/hardware/arx_r5/utils/white_balance.yaml}"
SHOW_NODE_HELP=0
...
warn() {
  printf '[arx_r5-run] WARN: %s\n' "$*" >&2
}

die() {
  printf '[arx_r5-run] ERROR: %s\n' "$*" >&2
  exit 1
}
...
run_python() {
  if [[ "${ARX_R5_FILTER_SDK_BANNER}" == "0" ]]; then
    "${ARX_R5_PYTHON_BIN}" -X faulthandler "$@"
    return
  fi
...
check_arx_r5_sdk() {
  ...
  [[ -n "${sdk_module}" ]] || die \
    "ARX R5 Python SDK is not built; run examples/hardware/arx_r5/SDK/ARX_R5_python/build.sh"
  run_python -c "import bimanual; from bimanual import SingleArm"
}
...
wait_for_device() {
  local device="$1"
  local deadline=$((SECONDS + ARX_R5_CAN_WAIT_SECONDS))
...
start_can_mapping() {
  ...
  log "starting ${device} as ${iface} with slcand -${ARX_R5_CAN_SLCAND_SPEED}"
  stop_matching_slcand "${device}" "${iface}"
  run_root slcand -o -f "-${ARX_R5_CAN_SLCAND_SPEED}" "${device}" "${iface}"
  sleep 0.5
...
init_arx_r5_can() {
  ...
  [[ "${INIT_ARX_R5_CAN}" != "0" ]] || {
    log "CAN setup skipped (INIT_ARX_R5_CAN=0)"
    return
  }
...
enable_arx_r5_arms() {
  [[ "${ENABLE_ARX_R5_ARMS}" != "0" ]] || {
    log "ARX arm enable skipped (ENABLE_ARX_R5_ARMS=0)"
    return
  }

  ARX_R5_ENABLE_SPECS="left_arm:${LEFT_CAN_PORT_VALUE};right_arm:${RIGHT_CAN_PORT_VALUE}" \
  ARX_R5_DISABLED_ARMS="${DISABLED_ARMS_VALUE}" \
  ARX_R5_ARM_TYPE="${ARM_TYPE:-0}" \
  run_python - <<'PY'
import os
...
disabled = disabled_groups(os.environ.get("ARX_R5_DISABLED_ARMS", ""))
arm_type = int(os.environ["ARX_R5_ARM_TYPE"])
enabled = 0
for spec in os.environ["ARX_R5_ENABLE_SPECS"].split(";"):
    group_name, can_port = spec.split(":", 1)
    if group_name in disabled:
        print(f"[arx_r5-run] {group_name} enable skipped: disabled")
        continue
    ...
    enabled += 1
    print(f"[arx_r5-run] {group_name} enabled on {can_port}")

if enabled == 0:
    print("[arx_r5-run] no ARX arms enabled")
PY
}
...
if [[ "${SHOW_NODE_HELP}" == "1" ]]; then
  run_python examples/hardware/arx_r5/node.py --help
  exit
fi
...
run_python examples/hardware/arx_r5/node.py \
