...
```bash
cd examples/hardware/arx_r5/SDK/ARX_R5_python
...
```bash
bash examples/hardware/arx_r5/run_hardware.sh \
...
```bash
bash examples/hardware/arx_r5/run_fake_node.sh
...
```bash
bash examples/hardware/arx_r5/utils/calibrate_white_balance_x3.sh
```

You can also select one camera from the command line:

```bash
bash examples/hardware/arx_r5/utils/calibrate_white_balance_x3.sh 2
```

Each run also saves one calibration frame per camera under
`work_dirs/arx_r5_orbbec_white_balance/<timestamp>/`.
...
```bash
bash examples/hardware/arx_r5/run_hardware.sh \
  --orbbec-white-balance-file examples/hardware/arx_r5/utils/white_balance.yaml
```
...
Set `INIT_ARX_R5_CAN=0` to skip CAN setup when the interfaces are already managed
outside this script.
...
```bash
ARM_TYPE=0 GRIPPER_OPEN_POS=5.0 bash examples/hardware/arx_r5/run_hardware.sh
```

`node.py` enables each connected arm from the long-lived `ArxR5DualArm` instance by
...
```bash
DISABLED_ARMS=left_arm bash examples/hardware/arx_r5/run_hardware.sh

# Equivalent CLI form:
bash examples/hardware/arx_r5/run_hardware.sh --disabled-arm left_arm
```
