...
# [0, ARX_R5_GRIPPER_OPEN_POS]. Calibrate per gripper via --gripper-open-pos.
ARX_R5_GRIPPER_OPEN_POS = 5.0
ARX_R5_GRIPPER_COMMAND_DELTA = 0.02
ARX_R5_RETRY_INTERVAL_SEC = 5.0
...
class ArxR5DualArm:
    ...
    def __init__(self, config: ArxR5ArmConfig) -> None:
        self._config = config
    ...
    def _mark_offline(self, group_name: str, exc: Exception) -> None:
        self._offline_until[group_name] = time.monotonic() + ARX_R5_RETRY_INTERVAL_SEC
        self._arms.pop(group_name, None)
        logger.warning(
            "ARX %s offline; using cached qpos for %.1f s: %s",
            group_name,
            ARX_R5_RETRY_INTERVAL_SEC,
            exc,
        )
    ...
    def _publish_gripper_if_needed(self, group_name: str, gripper_scalar: float) -> None:
        if group_name in self._last_gripper_command:
            ...
            if delta < ARX_R5_GRIPPER_COMMAND_DELTA:
                return
