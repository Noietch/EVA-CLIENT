...
DEFAULT_ARX_R5_SDK_DIR = Path(__file__).resolve().parent / "SDK" / "ARX_R5_python"
DEFAULT_ALICIA_PORT = ""
...
def prepare_arx_r5_runtime_paths(sdk_dir: Path) -> None:
...
def map_alicia_to_arx_r5_joints(alicia_joints: np.ndarray) -> np.ndarray:
    ...
    Returns:
        arx_r5_joints: [6] ARX R5 joint targets in radians.
    """
    joints = np.asarray(alicia_joints, dtype=float)
    if joints.shape[0] < 6:
        raise ValueError(f"Expected at least 6 Alicia joints, got {joints.shape}")
    arx_r5_joints = joints[:6] * ALICIA_MOTOR_DIR
    return np.clip(arx_r5_joints, ARX_R5_JOINT_LIMITS[:, 0], ARX_R5_JOINT_LIMITS[:, 1])


def read_arx_r5_joints(arm: Any) -> np.ndarray:
...
def calibrate_arx_r5_joint_offset(alicia_joints: np.ndarray, arx_r5_joints: np.ndarray) -> np.ndarray:
    ...
    Args:
        alicia_joints: [6] Alicia-D joint angles in radians.
        arx_r5_joints: [6] current ARX R5 joint angles in radians.

    Returns:
        offset: [6] offset added after applying ALICIA_MOTOR_DIR.
    """
    return np.asarray(arx_r5_joints, dtype=float)[:6] - map_alicia_to_arx_r5_joints(alicia_joints)
...
def create_arx_r5_arm(sdk_dir: Path, can_port: str, arm_type: int) -> Any:
    prepare_arx_r5_runtime_paths(sdk_dir)
...
def run_teleop(args: argparse.Namespace) -> None:
    if not args.dry_run:
        prepare_arx_r5_runtime_paths(args.arx_r5_sdk_dir)
    alicia_ports = resolve_alicia_group_ports(
        args.left_alicia_port,
        args.right_alicia_port,
        args.alicia_port,
    )
    can_ports = resolve_arx_r5_can_ports(
        args.left_can_port,
        args.right_can_port,
        args.can_port,
    )
    alicia_arms: dict[str, Any] = {}
    arx_r5_arms: dict[str, Any] = {}
    ...
    try:
        ...
        arx_r5_joints: dict[str, np.ndarray] = {}
        ...
        for group_name in GROUP_NAMES:
            ...
            if not args.dry_run:
                arx_r5_arms[group_name] = create_arx_r5_arm(
                    args.arx_r5_sdk_dir,
                    can_ports[group_name],
                    args.arm_type,
                )
                joint_offsets[group_name] = calibrate_arx_r5_joint_offset(
                    alicia_joints[group_name],
                    read_arx_r5_joints(arx_r5_arms[group_name]),
                )
            else:
        ...
        while True:
            for group_name in GROUP_NAMES:
                ...
                arx_r5_arm = arx_r5_arms[group_name] if group_name in arx_r5_arms else None
                if arx_r5_arm is not None:
                    arx_r5_arm.set_joint_positions(np.array(arx_r5_joints[group_name], dtype=float))
                    if not args.no_gripper:
                        alicia_gripper = alicia_grippers[group_name]
                        if alicia_gripper is not None:
                            arx_r5_arm.set_catch_pos(
            ...
            if args.print_interval > 0 and now >= next_print:
                print(
                    ...
                    f"left_arx={format_joints(arx_r5_joints['left_arm'])} "
                    "right_alicia="
                    f"{format_joints(alicia_joints['right_arm'])} "
                    f"right_arx={format_joints(arx_r5_joints['right_arm'])}"
                )
        ...
        for arx_r5_arm in arx_r5_arms.values():
            arx_r5_arm.protect_mode()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        dest="arx_r5_sdk_dir",
        type=Path,
        default=DEFAULT_ARX_R5_SDK_DIR,
    )
