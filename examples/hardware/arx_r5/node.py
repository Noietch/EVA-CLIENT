...
from examples.hardware.arx_r5.camera import (  # noqa: E402
    OrbbecCameraCache,
    OrbbecCameraSpec,
    apply_orbbec_white_balances,
    default_arx_r5_orbbec_camera_specs,
    load_orbbec_white_balances,
    merge_orbbec_camera_specs,
    parse_orbbec_camera_specs,
    parse_resolution,
)
from examples.hardware.arx_r5.robot import (  # noqa: E402
    ARM_DOF,
    ARX_R5_GRIPPER_OPEN_POS,
    DEFAULT_ARM_TYPE,
...
from examples.hardware.arx_r5.teleop import (  # noqa: E402
    ARX_R5_JOINT_LIMITS,
    DEFAULT_ALICIA_PORT,
    DEFAULT_ALICIA_READER,
    DEFAULT_LEFT_ALICIA_PORT,
    DEFAULT_RIGHT_ALICIA_PORT,
    calibrate_arx_r5_joint_offset,
    create_alicia_robot,
    map_alicia_to_arx_r5_joints,
    read_alicia_state,
...
def build_arx_r5_fk_solver() -> Any:
    import robots  # noqa: F401
    from core.registry import ROBOT_REGISTRY

    groups = [np.zeros(GROUP_DOF, dtype=np.float32) for _ in GROUP_NAMES]
    return ROBOT_REGISTRY.build("arx_r5").build_kinematics(initial_qpos_groups=groups)


@dataclasses.dataclass(frozen=True)
class ArxR5ZmqConfig:
...
class ArxR5TeleopSource:
    ...
    def calibrate_delta(self, robot_qpos: np.ndarray) -> None:
        for group_name in GROUP_NAMES:
            ...
            self._joint_offsets[group_name] = calibrate_arx_r5_joint_offset(
                joints,
    ...
    def read_action_qpos(self, robot_qpos: np.ndarray) -> np.ndarray:
        ...
        for group_name in GROUP_NAMES:
            ...
            arm_joints = np.clip(
                map_alicia_to_arx_r5_joints(joints) + self._joint_offsets[group_name],
...
class ArxR5ZmqNode:
    ...
    def __init__(self, config: ArxR5ZmqConfig, camera_endpoint: str = "") -> None:
        ...
        logger.info(
            "ARX R5 ZMQ node ready: obs_pub=%s action_sub=%s can_ports=%s "
    ...
    def _build_camera_cache(self, config: ArxR5ZmqConfig) -> Any:
    ...
    def start_collection(self, control_source: str = COLLECTION_CONTROL_TRANSPORT) -> None:
        if control_source not in COLLECTION_CONTROL_SOURCES:
            raise ValueError(f"Unsupported ARX R5 collection control source: {control_source!r}")
        ...
        if control_source == COLLECTION_CONTROL_CLIENT:
            self._collection_active = True
            logger.info("ARX R5 collection started with EVA client control")
            return
        try:
    ...
    def start_hil(self, mode: str) -> None:
        if self._config.passive_collection:
            self._hil_error = "ARX R5 passive collection mode cannot control HIL"
            return
    ...
    def _fk(self, qpos: np.ndarray) -> np.ndarray:
        if self._fk_solver is None:
            self._fk_solver = build_arx_r5_fk_solver()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gripper-open-pos",
        type=float,
        default=ARX_R5_GRIPPER_OPEN_POS,
...
def build_config(args: argparse.Namespace) -> ArxR5ZmqConfig:
    can_ports = {
    ...
    default_cameras = default_arx_r5_orbbec_camera_specs(
