...
def test_history_uses_qc_for_reviewed_episode_and_capture_quality_for_retake(tmp_path):
    ...
    (meta / "episodes.jsonl").write_text(
        json.dumps(dict(episode_index=0, tasks=["pick"], slot_id="SLOT-1", quality="red", length=3))
        + "\n"
        + json.dumps(
            dict(episode_index=1, tasks=["pick"], slot_id="SLOT-1", quality="green", length=3)
        )
        + "\n"
    )
    qc = tmp_path / "canonical-qc.jsonl"
    qc.write_text(json.dumps(dict(episode_index=0, slot_id="SLOT-1", qc_verdict="pass")) + "\n")
    rows = load_episode_history(tmp_path, qc_path=qc)["episodes"]
    assert rows[0]["qc_verdict"] == "pass"
    assert "qc_verdict" not in rows[1]
    qc.write_text(json.dumps(dict(episode_index=0, slot_id="SLOT-1", qc_verdict="fail")) + "\n")
...
def test_qc_matches_reindexed_canonical_capture_without_marking_new_retake(tmp_path):
    ...
    (runtime / "episodes.jsonl").write_text(json.dumps(original) + "\n" + json.dumps(retake) + "\n")
    (canonical / "episodes.jsonl").write_text(json.dumps({**original, "episode_index": 0}) + "\n")
