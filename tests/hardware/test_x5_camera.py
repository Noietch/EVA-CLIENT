...
from examples.hardware.x5.camera import (
    DEFAULT_X5_D405_CAMERAS,
    DEFAULT_X5_D405_PROFILE_PATH,
    X5_CAMERA_KEYS,
    RealSenseCameraSpec,
...
def test_default_d405_mapping_uses_stable_serials() -> None:
    ...
    assert tuple(spec.image_key for spec in specs) == X5_CAMERA_KEYS
    assert tuple(spec.serial for spec in specs) == tuple(DEFAULT_X5_D405_CAMERAS.values())
...
def test_d405_profile_has_all_fixed_camera_keys() -> None:
    profiles = load_realsense_color_profiles(DEFAULT_X5_D405_PROFILE_PATH)

    assert tuple(profiles) == X5_CAMERA_KEYS
...
def test_d405_day_profile_uses_shorter_exposure_than_night() -> None:
    ...
    assert all(day[key].exposure < night[key].exposure for key in X5_CAMERA_KEYS)
...
def test_realsense_spec_validation() -> None:
    ...
    with pytest.raises(ValueError, match="Unknown X5 camera key"):
        parse_realsense_camera_specs(
            ["unknown=index:0"],
            resolution=(640, 480),
            fps=30,
