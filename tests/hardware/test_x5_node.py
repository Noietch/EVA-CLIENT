...
from examples.hardware.x5.node import X5ZmqNode
from transport.zmq import unpack_observation


def test_x5_hardware_logging_keeps_native_output_visible() -> None:
    code = """
import os
from examples.hardware.x5.node import configure_hardware_logging
...
def test_x5_initializes_fk_before_native_hardware_workers(monkeypatch) -> None:
    events: list[str] = []
    ...
    build_fk_solver = X5ZmqNode._build_fk_solver
    monkeypatch.setattr(
        X5ZmqNode,
        "_build_fk_solver",
    ...
    monkeypatch.setattr(
        "examples.hardware.x5.node.ROBOT_REGISTRY.build",
        lambda name: SimpleNamespace(
            build_kinematics=lambda **kwargs: FakeFkSolver(),
        ),
    )
    monkeypatch.setattr(
        "examples.hardware.x5.node.X5DualArm",
    ...
    config = SimpleNamespace(
        observation_endpoint="inproc://x5-test-observation",
        action_endpoint="inproc://x5-test-action",
        can_ports={},
        realsense_cameras=(),
...
def test_x5_fk_solver_can_be_seeded_from_both_live_arm_states(monkeypatch) -> None:
    ...
    X5ZmqNode._build_fk_solver([left, right])
...
def test_x5_observation_publishes_dual_arm_state_and_camera_image() -> None:
    node = object.__new__(X5ZmqNode)
